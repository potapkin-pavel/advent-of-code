...
object Main {
  def main(args: Array[String]): Unit = {
    val lines = FileReader.getLines("src/main/resources/day3/input")
    // Part One
    println(
      lines
        .map(split)
        .map(takeSameItemFromBoth)
        .map(c => getMapOfCases.getOrElse(c, 0))
        .sum
    )
    // Part Two
    println(
      collectByThree(lines)
        .map(takeSameItemFromTupleOfThree)
        .map(c => getMapOfCases.getOrElse(c, 0))
        .sum
    )
  }
  ...
  def split(string: String): (String, String) = {
    val halfOfLength = string.length / 2
    (string.take(halfOfLength), string.drop(halfOfLength))
  }

  def collectByThree(listOfStrings: List[String]): List[(String, String, String)] = {
    @tailrec
    def loop(listOfStrings: List[String], accList: List[(String, String, String)]): List[(String, String, String)] =
      listOfStrings match {
        case Nil                 => accList
        case a :: b :: c :: list => loop(list, accList :+ (a, b, c))
      }

    loop(listOfStrings, List.empty[(String, String, String)])
  }

  def takeSameItemFromBoth(tupleOfTwo: (String, String)): Char = tupleOfTwo match {
    case (a, b) =>
      {
        for {
          char <- a if b.contains(char)
        } yield char
      }.toSet.head
  }

  def takeSameItemFromTupleOfThree(tupleOfThree: (String, String, String)): Char = tupleOfThree match {
    case (a, b, c) =>
      {
        for {
          char <- a if b.contains(char) && c.contains(char)
        } yield char
      }.toSet.head
  }

  val getMapOfLowercase: Map[Char, Int] = ('a' to 'z').zip(1 to 26).toMap

  val getMapOfUppercase: Map[Char, Int] = ('A' to 'Z').zip(27 to 52).toMap

  val getMapOfCases: Map[Char, Int] = getMapOfLowercase ++ getMapOfUppercase
}
