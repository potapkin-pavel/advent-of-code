...
object Main {
  def main(args: Array[String]): Unit = {
    val lines = FileReader.getLines("src/main/resources/day4/input")
    // Part One
    println(
      lines
        .map(splitLineByComa)
        .map { case (a, b) => (parseStringToRange(a), parseStringToRange(b)) }
        .map { case (range1, range2) =>
  ...
  def splitLineByComa(line: String): (String, String) = {
    // suppose string looks like "first part,second part"
    val split = line.split(",")
    (split(0), split(1))
  }

  def parseStringToRange(string: String): Range.Inclusive = {
    // suppose string looks like "2-5"
