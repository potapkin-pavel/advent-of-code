...
/*
  ...
  been enough characters received yet to find the marker. The first time
  a marker could occur is after the fourth character is received, making the
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    val listOfChars = FileReader.getListOfChars("src/main/resources/day6/input")
    // Part One
    println(findMarker(listOfChars))
    // Part Two
    println(findMarker(listOfChars, 14))
  }

  def findMarker(listOfChars: List[Char], length: Int = 4): Int = {
    @tailrec
    def loop(listOfChars: List[Char], acc: Int): Int = {
      if (checkIfAllDistinct(listOfChars.take(length))) acc
      else loop(listOfChars.drop(1), acc + 1)
    }
    loop(listOfChars, length)
  }

  def checkIfAllDistinct[A](list: List[A]): Boolean = list.distinct.size == list.size
