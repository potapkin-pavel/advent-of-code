...
/*
  ...
  etc. that they've brought with them, one item per line.
  ...
  to know how many Calories are being carried by the Elf
...
object Main {
  ...
  val getLines: List[String] = {
    val bufferedSource: BufferedSource =
      scala.io.Source.fromFile("src/main/resources/day1/input")
  ...
  def main(args: Array[String]): Unit = {
    // Part One
    println(split(getLines).map(_.map(_.toInt).sum).max)
    // Part Two
    println(split(getLines).map(_.map(_.toInt).sum).sortWith(_ > _).take(3).sum)
  }
}
