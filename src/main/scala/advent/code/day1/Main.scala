...
/*
  ...
  boats approach land, the Elves begin taking inventory of their supplies.
  ...
  Elf to ask: they'd like
  to know how many Calories are being carried by the Elf carrying the most
...
object Main {
  ...
  def split(lines: List[String]): List[List[String]] = {
    ...
    def loop(lines: List[String], acc: List[List[String]]): List[List[String]] =
      lines match {
        case Nil                  => acc
        case x :: xs if x.isBlank => loop(xs, List.empty[String] +: acc)
        case x :: xs              => loop(xs, (acc.head :+ x) +: acc.tail)
      }
    loop(lines, List(List.empty[String]))
  }

  def main(args: Array[String]): Unit = {
    val lines = FileReader.getLines("src/main/resources/day1/input")
    // Part One
    println(split(lines).map(_.map(_.toInt).sum).max)
    // Part Two
    println(split(lines).map(_.map(_.toInt).sum).sortWith(_ > _).take(3).sum)
  }
}
