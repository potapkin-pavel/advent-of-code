...
case class ChangeDirectory(directory: String) extends Command
case object ListFiles extends Command

sealed trait TerminalLine
case class Cmd(cmd: Command) extends TerminalLine
case class Directory(name: String) extends TerminalLine
case class File(size: Int, name: String) extends TerminalLine

case class DirectoryStructure(
    name: String,
    children: mutable.Map[String, DirectoryStructure],
    files: mutable.Map[String, Int],
    parent: Option[DirectoryStructure]
)

object Main {
  private val lines = FileReader.getLines("src/main/resources/day7/input")

  def main(args: Array[String]): Unit = {
    val rootDir = DirectoryStructure("/", mutable.Map.empty, mutable.Map.empty, None)
    // Part One
    buildState(lines.map(parse), rootDir, rootDir)
    println(collectSizes(rootDir, _ < 100_000).sum)
    // Part Two
    val result2 = {
  ...
  @tailrec
  def buildState(input: List[TerminalLine], currentDir: DirectoryStructure, rootDir: DirectoryStructure): Unit = {
    input match {
      case Cmd(ChangeDirectory("/")) :: t => buildState(t, rootDir, rootDir)
      case Cmd(ChangeDirectory("..")) :: t =>
        currentDir.parent match {
          case Some(parentDir) => buildState(t, parentDir, rootDir)
        }
      case Cmd(ChangeDirectory(name)) :: t => buildState(t, currentDir.children(name), rootDir)
      ...
      case Directory(name) :: t =>
        currentDir.children.put(
          name,
          DirectoryStructure(name, mutable.Map.empty, mutable.Map.empty, Some(currentDir))
        )
        buildState(t, currentDir, rootDir)
      case Nil => ()
    }
  ...
  def directorySize(directoryStructure: DirectoryStructure): Int =
    directoryStructure.files.values.sum + directoryStructure.children.values.map(directorySize).sum

  def collectSizes(dir: DirectoryStructure, sizeFilter: Int => Boolean): Iterable[Int] = {
    val size = directorySize(dir)
    val children = dir.children.values.flatMap(collectSizes(_, sizeFilter)).toList
    if (sizeFilter(size)) size :: children else children
  }

  def parse(string: String): TerminalLine = string match {
